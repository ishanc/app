...
class CSodTransformer:
    ...
    def _infer_file_type(self, filepath: str) -> str:
        ...
        for file_type in self.field_mappings.keys():
            if file_type.lower() == base_name.lower():
                return file_type
    ...
    def transform_file(self, input_path: str, output_path: str):
        ...
        try:
            ...
            self.logger.info(f"File read successfully. Columns found: {df.columns.tolist()}")
            
            # Create a new dataframe for the transformed data
            result_data = {}
            
            # Get the rules for this file type
            file_rules = self.field_rules.get(file_type, {})
            field_mappings = self.field_mappings.get(file_type, {})
            
            # Process each CSOD field
            for csod_field, rules in file_rules.items():
                # Find corresponding SumTotal field if it exists
                sumtotal_field = next(
                    (st for st, cs in field_mappings.items() if cs == csod_field), 
                    None
                )
                
                if sumtotal_field and sumtotal_field in df.columns:
                    self.logger.info(f"Transforming field: {sumtotal_field} -> {csod_field}")
                    # Transform mapped field
                    result_data[csod_field] = [
                        self._validate_and_transform_value(
                            value, rules, csod_field, input_path, idx
                        )
                        for idx, value in enumerate(df[sumtotal_field])
                    ]
                else:
                    if sumtotal_field:
                        self.logger.warning(f"SumTotal field {sumtotal_field} not found in input file")
                    self.logger.info(f"Using default/empty values for {csod_field}")
                    # Handle unmapped field
                    result_data[csod_field] = [
                        rules.get("default_value", "")
                    ] * len(df)
    ...
    def transform_directory(self, input_dir: str, output_dir: str):
        """Transform all files in a directory structure."""
        self.logger.info(f"Starting directory transformation from {input_dir} to {output_dir}")
        self.logger.info(f"Available file type mappings: {list(self.field_mappings.keys())}")
        ...
        try:
            for root, _, files in os.walk(input_dir):
                ...
                if not excel_files:
                    self.logger.info(f"No Excel files found in {root}")
                    continue
                    
                self.logger.info(f"Found Excel files: {excel_files}")
                ...
                for file in excel_files:
                    ...
                    # Add -CSOD suffix to filename and change extension to .csv
                    filename_without_ext = os.path.splitext(file)[0]
                    new_filename = f"{filename_without_ext}-CSOD.csv"
                    
                    output_path = os.path.join(output_dir, rel_path, new_filename)
                    
                    self.logger.info(f"Transforming {input_path} -> {output_path}")
                    self.transform_file(input_path, output_path)
                
        except Exception as e:
            self.logger.error(f"Error in directory transformation: {str(e)}")
            raise
